    ...
    def log_to_dynamodb(self, session_id: str, metrics_data: Dict):
        try:
            ...
            item = {
                ...
                'evaluation_metrics': {
                    'M': {
                        'ground_truth_matches': {'N': str(Decimal(str(metrics_data['ground_truth_matches'])))},
                        'similarity_scores': {
                            'L': [{'N': str(Decimal(str(score)))} for score in metrics_data['similarity_scores']]
                        },
                        'rouge_scores': {
                            'M': {
                                'rouge1': {'N': str(Decimal(str(metrics_data['average_rouge_scores']['rouge1'])))},
                                'rouge2': {'N': str(Decimal(str(metrics_data['average_rouge_scores']['rouge2'])))},
                                'rougeL': {'N': str(Decimal(str(metrics_data['average_rouge_scores']['rougeL'])))},
                            }
                        },
                        'bleu_score': {'N': str(Decimal(str(metrics_data['average_bleu_score'])))},
                    }
                },
                'qa_pairs': {
                    'L': [
                        {
                            'M': {
                                ...
                                'rouge_scores': {
                                    'M': {
                                        'rouge1': {'N': str(Decimal(str(qa['rouge_scores']['rouge1'])))},
                                        'rouge2': {'N': str(Decimal(str(qa['rouge_scores']['rouge2'])))},
                                        'rougeL': {'N': str(Decimal(str(qa['rouge_scores']['rougeL'])))},
                                    }
                                },
                                'bleu_score': {'N': str(Decimal(str(qa['bleu_score'])))},
                            }
                        }
...
class GemmaDoRAFineTunedLLM:
    def __init__(self, model_path="/Users/aafrinshaheen/Desktop/Domain_expert/gemma-2-model"):
...
Use this metadata to **tailor your question** to the author’s expertise and the project's business context. Your goal is to surface **one insightful and non-repetitive question** that reflects a deep understanding of both the content and the metadata context.
...
class Llama3LLM:
   def __init__(self, model_path="/Users/aafrinshaheen/Desktop/Domain_expert/llama-base-model", model_name="llama-3.2-base"):
...
class Llama3LoraFineTunedLLM:
   def __init__(self, model_path="/Users/aafrinshaheen/Desktop/Domain_expert/llama-3.2-lora-model", model_name="llama-lora-finetuned"):
       self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
       self.tokenizer = AutoTokenizer.from_pretrained(model_path)
       if self.tokenizer.pad_token is None:
           self.tokenizer.pad_token = self.tokenizer.eos_token
   ...
   def invoke(self, prompt: str):
       try:
           ...
           logger.error(f"Fine-tuned LLaMA error: {e}")
           return AIMessage(content="Error generating response from Fine-Tuned LLaMA3.")
...
or multiple questions. Just one clear, focused question based on the document context."""
    ...
    Your question:"""
        ...
        elif "gemma-finetuned-dora" in model_name.lower():
            self.llm = GemmaDoRAFineTunedLLM(model_path="/Users/aafrinshaheen/Desktop/Domain_expert/gemma-2-model")
        ...
        # Path to your fine-tuned model 
            model_path = "/Users/aafrinshaheen/Desktop/Domain_expert/qwen-0.5b-dora-adapter-5epochs"  
            self.llm = QwenFinetunedLLM(model_path=model_path)
        
        # Custom prompt template for fine-tuned model
...
YOUR UNIQUE TECHNICAL QUESTION:"""
           ...
           # call the hosted LLaMA inference API
           self.llm = Llama3LLM(model_path="/Users/aafrinshaheen/Desktop/Domain_expert/llama-base-model")
           self.prompt_template = """### Instruction:
       ...
       ### Response:"""
            ...
            self.llm = Llama3LoraFineTunedLLM(model_path="/Users/aafrinshaheen/Desktop/Domain_expert/llama-3.2-lora-model")
            self.model_name = model_name
